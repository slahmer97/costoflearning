...
task_id = "110"
data = pd.read_csv("task-{}/dead1.csv".format(task_id))
size = 200000
ofb = pandas.DataFrame(
    {
        "packet_death": np.random.choice(data["{}-ofb".format(task_id)].to_numpy().reshape(-1), size=size),
        "strategy": ["ofb"] * size
    }
)
print(len(ofb))
dynamic = pandas.DataFrame(
    {
        "packet_death": np.random.choice(data["{}-dynamic".format(task_id)].to_numpy().reshape(-1), size=size)
        ,
...
always1 = pandas.DataFrame(
    {
        "packet_death": np.random.choice(data["{}-static-1".format(task_id)].to_numpy().reshape(-1), size=size),
...
every10 = pandas.DataFrame(
    {
        "packet_death": np.random.choice(data["{}-every-10".format(task_id)].to_numpy().reshape(-1), size=size),
...
every100 = pandas.DataFrame(
    {
        "packet_death": np.random.choice(data["{}-every-100".format(task_id)].to_numpy().reshape(-1), size=size),
...
ofb = ofb[~ofb['packet_death'].isna()]
dynamic = dynamic[~dynamic['packet_death'].isna()]
always1 = always1[~always1['packet_death'].isna()]
every10 = every10[~every10['packet_death'].isna()]
every100 = every100[~every100['packet_death'].isna()]
...
ax = sns.barplot(data=data, x="strategy", y="packet_death", ci=99.999, palette=colors, dodge=False)
...
# plt.legend()
tikzplotlib.save("packet-dead-task{}.tex".format(task_id))
plt.show()
