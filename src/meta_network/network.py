...
from src.meta_network.flow_gen import AggregateOnOffFlowGenerator
from src.meta_network.netqueue import NetQueue
import numpy as np
import gym


class MetaNetwork(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        self.slices = []
        for i in range(2):
            self.slices.append(NetQueue(sim=self._sim, type=i))

        self.normaliser = np.array([1, 1, 1, 1, 1, 1])

    def move_environment(self, **kwargs):
        self.normaliser = np.array(
            [float(kwargs["max_users:0"]), float(kwargs["max_users:1"]), 1500.0, 1500.0, self._sim.RESOURCES_COUNT,
             self._sim.RESOURCES_COUNT])
        self.slice_0_flows = [
            {
        ...
        for conf in self.slice_0_flows:
            self.generators.append(AggregateOnOffFlowGenerator(**conf, sim=self._sim))
    ...
    def init_resources(self):
        res = 10
        self.slices[0].allocate_resource(self._sim.RESOURCES_COUNT - res)
    ...
    def step(self, action=None, greedy_selection=None):
        def reward_func4(obj, drop1, dead2, cum_cost2, resent2, served2):
            ...
            coeiff = self._sim._cost_weights
            return coeiff[0] * normalized_cost1 + coeiff[1] * normalized_cost2, normalized_cost1, normalized_cost2
        ...
        if action == 0:
            ...
            self.reset_values()
            self.action1 += 1
            self.slices[0].allocate_resource(min(self._sim.RESOURCES_COUNT, self.slices[0].allocated_resources + 1))
            self.slices[1].allocate_resource(self._sim.RESOURCES_COUNT - self.slices[0].allocated_resources)
    ...
    def reset(self, **kwargs):
        ...
        self.slices = []
        for i in range(2):
            self.slices.append(NetQueue(type=i, sim=self._sim))
        self.init_resources()
        self.state = [lambda_1, lambda_2, len(self.slices[0]), len(self.slices[1]), self.slices[0].allocated_resources,
                      self.slices[1].allocated_resources]
        return np.array(self.state)
    ...
    def get_expected_rates(self):
        p_on0 = self._sim.Transitions[0][0][1] / (self._sim.Transitions[0][0][1] + self._sim.Transitions[0][1][0])
        p_on1 = self._sim.Transitions[1][0][1] / (self._sim.Transitions[1][0][1] + self._sim.Transitions[1][1][0])
