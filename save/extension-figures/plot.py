...
def plot_average_queue_sizes(files, queue=1):
    X = np.arange(0, 4000)
    indices = np.arange(0, 4000, 10)

    print(X)
    for i, file in enumerate(files):
        currentFileDF = pd.read_csv("{}.csv".format(file))
        avg_queue_sizes = currentFileDF["slice{}:queue_sizes".format(queue)].rolling(300).mean().to_numpy() * 1500.0
        sns.lineplot(x=np.take(X, indices), y=np.take(avg_queue_sizes, indices), sort=False, lw=1)
...
def plot_average_latecy(files, queue=0):
    ...
    for i, file in enumerate(files):
        currentFileDF = pd.read_csv("{}.csv".format(file))
        avg_drop_rate = currentFileDF[metric].ewm(span=200,
                                                  adjust=True).mean().to_numpy()
...
all_files = ["dynamic-simpleReward", "outOfBand", "static(1)-simple-reward", "static(2)-simple-reward"]
# plot_agent_reward_bins(files=["0.csv", "1.csv", "2.csv", "3.csv"])
plot_average_queue_sizes(files=all_files, queue=0)
