...
def single_task():
    ...
    task["resources_count"] = 14
    # task["max_users:1"] = 13
    coeif = 0.5
    task["cost_weights"] = [coeif, 1.0 - coeif]
    task_id = "{}-{}".format(idd, "static")
    ...
    # exit(0)
    env = Simulation(**task)
    greedy_policy = OnlyLearningPlanePolicy()  # OnlyLearningPlanePolicy() #GreedyPolicy()
    ...
    wandb.run.save()
    #dqn.eval_net.load_me("./ex-model/task{}-episode{}".format(idd, 80))
    #dqn.target_net.load_me("./ex-model/task{}-episode{}".format(idd, 80))
