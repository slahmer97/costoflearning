...
def run(**run_config):
    ...
    #dqn.eval_net.load_me("models/eval{}".format(200))
    #dqn.target_net.load_me("models/target{}".format(200))
    for i in range(episodes):
        if i in [700, 1400, 2100, 2800, 3500, 4200, 4900]:
            env.move_system()
...
def run_experiments():
    ...
    for i in range(0, len(c["use_greedy"])):
        ...
        torch.manual_seed(0)
        config = {
            "sim-id": "dynamic".format(c["learning_resources_count"][i]),
