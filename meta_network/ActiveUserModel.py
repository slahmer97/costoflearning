...
class ActiveUsers:
    def __init__(self,sim, max_users=2, process_name='A', slice=0):
        self._sim = sim
    ...
    def get_prob(self, j, h):
        ...
        for l in range(l_min, l_max + 1):
            ...
            term2 = np.power(self._sim.Transitions[self.slice][1][1], l) * np.power(self._sim.Transitions[self.slice][0][1], j - l) * np.power(self._sim.Transitions[self.slice][1][0], h - l) * \
                    np.power(self._sim.Transitions[self.slice][0][0], self.max_users - h - j + l)
            sum += term1 * term2
        return sum
